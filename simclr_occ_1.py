...
class Config:
    ...
    normal_path = os.path.join(root_path,'LYT')
...
# Load the 150th epoch checkpoint
checkpoint_filename = "checkpoint_epoch_150.pth"
checkpoint_filepath = os.path.join(checkpoint_path, checkpoint_filename)
simclr_model.load_state_dict(torch.load(checkpoint_filepath))
print(f"Loaded checkpoint at epoch 150 - {checkpoint_filepath}")


# Training loop
for epoch in range(150, EPOCHS + 1):
    t0 = time.time()
    running_loss = 0.0
    ...
    print(f'EPOCH: {epoch + 1} LOSS: {epoch_loss:.16f}, Time: {(time.time() - t0) / 60:.4f} mins')
    ...
    if epoch % 50 == 0:
        ...
        checkpoint_filepath = os.path.join(checkpoint_path, checkpoint_filename)
        torch.save(simclr_model.state_dict(), checkpoint_filepath)
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('CenterCropResize - Batch Size 16 - Pretraining Loss Over Time')
plt.savefig('CenterCropResize_16_Pretraining_loss_plot.png')



# torch.save(simclr_model.state_dict())

PATH = 'model_pretrained_centercropresize_batch16.pt'
